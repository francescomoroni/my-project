  ...
  return (
    <>
      <div className="pb-40 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-24 xl:gap-x-20 xl:gap-y-32 my-8 mx-4 sm:mx-8">
        {works.map((work, key) => {
          return (
              <Link href={`/pages/works/work/${work.linkPage}`} key={key} className="relative mx-auto h-[18rem] xl:h-[24rem] w-full group ">
                <Image
                ...
                <h1 className="group-hover:line-through transition-all duration-300 ease-in-out transform pt-8 pl-2  uppercase cursor-pointer sm:text-lg md:text-xl ">{work.titleWorkPage}</h1>
              </Link>
          )
        }
        )}
